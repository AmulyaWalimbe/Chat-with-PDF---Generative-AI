...
We built two separate interfaces:
  • Admin Interface that uploads the PDF, extracts the text, generates embeddings, and creates a FAISS vector index stored in Amazon S3, and
  • User Interface that loads this index, runs similarity search using FAISS, and sends the retrieved text to Claude for answer generation.
